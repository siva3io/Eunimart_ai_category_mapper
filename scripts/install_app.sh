...
rm -rf /etc/systemd/system/service-name-server.service
...
cat >> /etc/systemd/system/service-name-server.service <<EOT
[Unit]
...
ExecStart=/home/ubuntu/eunimart_access_control/scripts/run_server.sh
RemainAfterExit=no
Restart=on-failure
RestartSec=5s
User=root
...
systemctl enable service-name-server.service
systemctl start service-name-server.service
